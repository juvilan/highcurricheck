...
def check_curriculum(parsed):
    ...
    # D2: 기초교과(국어·수학·영어·한국사) 이수학점 ≤ 교과 총 이수학점의 50%.
    #
    # 한국사는 교과군이 '사회(역사/도덕 포함)'라 국·수·영 합산에서 빠진다. 과목명으로
    # 따로 더한다(교과군이 국·수·영으로 잡힌 경우는 중복 방지를 위해 제외).
    #
    # 상한은 교과총의 50%를 그때그때 계산한다. 예전에는 고정값 81을 썼는데,
    # 이건 174×50% − 한국사 6 을 미리 빼 둔 값이라 '교과총 174, 한국사 정확히 6'이라는
    # 두 가정이 박혀 있었다. 학교가 교과 175학점을 편성하거나 한국사 운영학점이 다르면
    # 기준 자체가 틀어진다.
    국영수행 = [x for x in rows if x['교과군'].replace(' ','') in ('국어','수학','영어')]
    국영수_rows = {x['row'] for x in 국영수행}
    한국사행 = [x for x in rows
              if '한국사' in (x['과목명'] or '') and x['row'] not in 국영수_rows]
    KSE = sum([(x['운영학점'] or 0) for x in 국영수행])
    한국사 = sum([(x['운영학점'] or 0) for x in 한국사행])
    기초 = KSE + 한국사
    교과총 = gyo or 174
    한도 = 교과총 * 0.5
    합계식 = f"국·수·영 {KSE:.0f} + 한국사 {한국사:.0f} = {기초:.0f}"
    # 운영학점 합은 '선택 조건 미반영 상한값'. 기초교과에 선택과목(공통이 아닌 유형)이나
    # 비고 '택' 표기가 있으면 한도 초과여도 FAIL이 아닌 WARN — 선택에 따라 실제 이수가
    # 줄 수 있어 수동확인이 맞다. (과목유형 공백표기에 견고)
    기초행 = 국영수행 + 한국사행
    has_choice = any(typ(x) not in ('공통', '') for x in 기초행) \
        or any('택' in (x['비고'] or '') for x in 기초행)
    if 기초 <= 한도:
        d2_res, d2_ev = "PASS", f"{합계식} (한도 {한도:.0f} 이하)"
    elif has_choice:
        d2_res = "WARN"
        d2_ev = (f"{합계식} = 선택/택 조건 미반영 상한값 (한도 {한도:.0f}). "
                 f"실제 이수는 선택에 따라 ≤{기초:.0f}일 수 있음 — 비고·특이사항 수동확인 필요")
    else:
        d2_res = "FAIL"
        d2_ev = f"{합계식} (선택조건 없음, 한도 {한도:.0f} 초과)"
    add("D2","D.균형","기초교과(국·수·영·한국사) ≤ 교과의 50%",
        f"≤ {한도:.0f}학점 (교과 {교과총:.0f}의 50%)", f"{기초:.0f}", d2_res, d2_ev)
    ...
    add("F4","F.수동확인","학기 단위 이수(완결성)","과목별 학기 내 완결 편성","수동확인 권장","INFO",
        "한 과목이 한 학기 안에서 완결되도록 편성됐는지(여러 학기에 쪼개 걸치지 않았는지)는 "
        "배당표만으로 자동 판정이 어렵습니다. 학기별 배치를 직접 확인하세요.")
    add("F5","F.수동확인","2022 개정 과목 적용","신규 과목명·편제 반영","수동확인 권장","INFO",
        "2015 개정 잔재 없이 2022 개정 신규 과목명·편제(과목 신설/통합/명칭 변경)가 맞게 "
        "반영됐는지 직접 확인 권장. (E1·E3는 표기·정규목록 대조만 자동 수행합니다.)")
    add("F6","F.수동확인","기록 형식 준수","선택과목 수·학점 표기 방식","수동확인 권장","INFO",
        "선택 과목 수 표기, ‘(택N)’ 등 학점 표기 방식이 지침이 요구하는 형식에 맞는지는 "
        "형식 판단이 필요해 직접 확인 권장.")
