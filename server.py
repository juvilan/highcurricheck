...
CHANGELOG = [
    {"date": "2026-06-04", "items": [
        "점검 항목 2개 추가 — 한국사 Ⅰ·Ⅱ 모두 편성·각 3학점, 체육 총 10학점 이상. (교육청 AI 점검표 17개 항목 대조 보완)",
        "‘검토 중…’ 로딩 표시 추가 — 업로드 검토·다시 검토 시 스피너 오버레이로 진행 중임을 표시(여러 파일 검토 시 멈춘 듯 보이던 문제 해소).",
...
def login():
    ...
    if request.method == "POST":
        ...
        if not username or not password:
            ...
            if u is None:  # 처음 보는 이름 → 즉시 가입(승인 불필요)
                db.create_user(username, generate_password_hash(password), approved=1)
